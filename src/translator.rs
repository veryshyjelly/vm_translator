...
impl Arithmetic {
    fn translate(self, index: &mut u64, filename: &str) -> Vec<String> {
        use Arithmetic::*;
...
impl Command {
    pub fn init() -> Vec<String> {
        let mut instructions = ["@256", "D=A", "@SP", "M=D"]
            ...
            .map(|x| x.to_string())
            .collect::<Vec<_>>();
        let mut call_sys = translate_call("Sys.init".into(), 0, "Sys.init$ret.0".into());
    ...
    pub fn translate(self, filename: &str, index: &mut u64) -> Vec<String> {
        use Command::*;
        match self {
            Arithmetic(operation) => operation.translate(index, filename),
            Push(segment, index) => translate_push(filename, segment, index),
            Pop(segment, index) => translate_pop(filename, segment, index),
            Label(label) => translate_label(label),
            Goto(label) => translate_goto(label),
            IfGoto(label) => translate_if(label),
            Function(name, local_var) => translate_function(name, local_var),
            Call {
                callee: name,
                arg_count,
                return_address,
            } => translate_call(name, arg_count, return_address),
...
fn translate_return() -> Vec<String> {
    let mut instructions = vec![
        "@LCL", "D=M", "@R13", "M=D", "@5", "A=D-A", "D=M", "@R14", "M=D", "@SP", "M=M-1", "A=M",
        "D=M", "@ARG", "A=M", "M=D", "D=A", "@SP", "M=D+1",
    ];
    ["@THAT", "@THIS", "@ARG", "@LCL"]
        .into_iter()
